...
<?php
	...
	$dateData=array_selectUpDown($transData,'日期',$_SESSION[endTime],$_SESSION[startTime]);
	if(empty($dateData)){
		//为空，则返回
		$_SESSION['intersourseData']=$notNullData;
		return;
	...
	//把时间改成字符串，把客户改成字符串
	function setTimeTypeToStr(&$data){
		foreach($data as $keys => $values){
			//print_r($values);
			echo "<br/>";
			$values['日期']= date("Y-m-d H:i:s",$values['日期']);
			//$values['捐赠时间']= date("Y-m-d H:i:s",1439510340);
			/*
			switch($values['客户类型']){
				case 0:
					$values['客户类型']='个人捐赠';
