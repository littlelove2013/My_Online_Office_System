...
<?php
	...
	//获取个人所管理的集体
		...
		//echo $current_file."<p>";
		$table="personal inner join company_have on personal_id=personal_personal_id
			  inner join company on company_company_id=company_id";
		$field="personal_id as id,personal_name as '机构成员'";
		...
		if(!empty($myCompany)){
			foreach($myCompany as $keys=>$values){
				$values['机构成员']="<a id='table_td' href='../index.php?customer_type=个人&customer_id=".$values[id]."' target='_top'>".$values['机构成员']."</a>";
				$values['操作']="
								<a id='table_td' href='insertUpdateManage/deleteP.php?customer_type=机构&company=company&id=".$_GET[customer_id]."&personal_id=".$values[id]."'>退出本公司</a>
								";
			...
			echo "<p>";
			$insert="(<a href='insertUpdateManage?customer_type=个人&company_id=".$_GET[customer_id]."'>添加新成员</a>)";
			$myCompanyShow->Display("机构成员".$insert);
			...
			echo "<p>";
			$myCompanyDonatedRecorderShow->Display("机构成员的捐赠记录");
		}else{
			//没有时采取的动作
			$data[]=array("还没有成员"=>"<a id='table_td' href='insertUpdateManage/index.php?customer_type=个人&company_id=".$_GET[customer_id]."'>现在去添加一个成员？</a>");
