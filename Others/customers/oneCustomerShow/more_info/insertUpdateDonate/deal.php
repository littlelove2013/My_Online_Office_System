<?php
	...
	if($_POST[update]=='修改'){
		//对捐赠数据
		//获取当前日期作为修改日期
		$insertDonatedFunds[donated_funds_lastedit_date]=time();
		$insertDonatedFunds[donated_funds_date]=strtotime($insertDonatedFunds[donated_funds_date]);
		//print_r($insertDonatedFunds);
	...
	if($_POST[insert]=='添加'){
		...
		$data[donated_funds_date]=strtotime($insertDonatedFunds[donated_funds_date]);
		...
		$data[donated_funds_lastedit_date]=time();
